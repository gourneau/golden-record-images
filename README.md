...
There are **116 photographs** bolted to the side of two spacecraft leaving the solar system. They
aren't stored as pictures — they're stored as **sound**, about eight minutes of harsh buzzing
pressed into a gold-plated copper phonograph record.
...
<p align="center"><b><a href="https://gourneau.github.io/golden-record-images/">Browse all 116 decoded images →</a></b></p>

---

## What's different here

**The pictures are cleaner than any previously published decode**, and the reason is a defect
nobody had identified: an error that **accumulates down each trace in proportion to the light
already sent**. It is the single mechanism behind both the brightness droop and the vertical
streaking that every earlier decode carries.

The record proves it itself. The slide mount appears **twice on every frame**, before the picture
and after it — one physical object, so both readings must agree. Shading would move both ends
alike; an accumulating error darkens only the end. Measured across twelve frames: the bottom mount
slope is **−0.284, negative on 12 of 12**; the top is +0.054, negative on 3.
[How it works →](docs/findings.md)

**The decode is at the physical limit, and that limit is not ours.** The recording chain could have
carried over 1000 elements along a stripe; the 1977 **camera** resolved 138–172. We already sample
1.4–1.9× finer than the camera could see. Everything it captured is in these pictures, and anything
sharper a program now produces is **invented, not recovered**.

**The machine learning is judged by prediction, not by prettiness.** What ships is Noise2Noise
trained on the record's own colour repeats — twenty scenes scanned three times, which is twenty
sets of independent noisy observations. It beats the raw plane on **19 of 19** unseen scenes and a
blur control on every one. [The four methods and nine nulls →](docs/ML_HANDOFF.md)
...
| **[What this decoder found](docs/findings.md)** | The measurements, in signal-processing detail. |
| **[The machine learning](docs/ML_HANDOFF.md)** | Four methods, one shipped, and the arbiter they were judged by. |
| **[What didn't work](docs/what-didnt-work.md)** | Nine ideas built, measured, rejected. A map of the holes in this signal. |
| **[What an alien could recover](ALIENS.md)** | Which parts a recipient could actually do, and where the record fails its reader. |
| **[Corrections and retractions](docs/corrections.md)** | Everything we got wrong, and findings that bear on Wikipedia and Commons. |

<p align="center">
  <img src="docs/img/calibration-barry.png" width="42%" alt="Calibration circle decoded with the 2017 method: an ellipse">
  <img src="docs/img/calibration-restored.png" width="42%" alt="Calibration circle decoded with this decoder: round">
</p>

*Image 1 is a circle — a test pattern the designers included so a recipient could check their own
work. Left: the 2017 method, an ellipse of axis ratio **1.0485** that had to be nudged by hand.
Right: this decoder at **1.0054**, no fudge factor. Both regenerated from the master by
`python -m pipeline.figures`, so neither can drift from what the code does.*
